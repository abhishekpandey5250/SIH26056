 ...
 */
export function validateTrafficRecord(record = {}, defaultDataEnv = 'REAL') {
  if (!record || typeof record !== 'object' || Array.isArray(record)) {
    return { record: null, error: 'Record must be a valid JSON object' };
  }

  const period = (typeof record.period === 'string' && record.period.trim()) || '2026-Q1';
...
export async function importTrafficRecords(rawRecords = [], options = {}) {
  ...
  rawRecords.forEach((raw, idx) => {
    const { record, error } = validateTrafficRecord(raw, defaultDataEnv);
    if (!record) {
      rejectedCount++;
...
async function main() {
  ...
  if (!fileArg) {
    console.log('Usage: node scripts/importDGCATraffic.js <filePath.csv|.json> [--mode=REAL|DEMO]');
    process.exit(0);
  }
  ...
  try {
    ...
    const summary = await importTrafficFile(fileArg, { dataEnvironment: modeArg });
