...
export class DailyRouteAggregationService {
  ...
  static processObservations(observations, targetDate, options = {}) {
    ...
    for (const group of groups.values()) {
      ...
      const eligibleFares = group.eligibleObservations
        .map((o) => o.fare?.observedFare)
        .filter((f) => typeof f === 'number');

      const representativeFare = calculateMedian(eligibleFares);
      const minFare = Math.min(...eligibleFares);
      const maxFare = Math.max(...eligibleFares);

      // Source / Airline breakdown calculation
      const sourceBreakdown = {};
      for (const obs of group.eligibleObservations) {
