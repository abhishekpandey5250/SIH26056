 ...
 */
export { normalizeWeights } from '../utils/weights.js';
...
export class AirfareIndexService {
  ...
  static async calculateAndPersistIndex(date, options = {}) {
    ...
    // Use the same source of truth as the dashboard and analytics endpoints.
    // This resolves the previous mismatch where the index read RouteWeight
    // documents while the dashboard derived its basket from DGCATraffic.
    const activeBasket = await RouteBasketService.getActiveBasket({ dataEnvironment });
    routeWeights = activeBasket.routes;
