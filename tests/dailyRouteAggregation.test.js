...
describe('Step 3: Route + Advance-Purchase Lead-Time Aggregation Engine', () => {
  ...
  describe('2. Methodology: Representative Median and Statistical Metrics', () => {
    ...
    it('excludes a clear IQR fare outlier before calculating the route median', () => {
      const observations = [5000, 5100, 5200, 5300, 25000].map((fare, index) => ({
        scrapedAt: `2026-09-04T0${index + 1}:00:00Z`,
        ...
        tripType: 'one-way',
        cabinClass: 'economy',
        fare: { observedFare: fare, currency: 'INR' },
        quality: { indexEligible: true, flags: [] },
        airline: 'Test Airline',
      }));

      const [aggregation] = DailyRouteAggregationService.processObservations(
        observations,
        '2026-09-04',
        { requireNonStop: true }
      );

      expect(aggregation.eligibleObservationCount).toBe(5);
      expect(aggregation.includedObservationCount).toBe(4);
      expect(aggregation.quality.outlierObservationCount).toBe(1);
      expect(aggregation.representativeFare).toBe(5150);
      expect(aggregation.maxFare).toBe(5300);
    });

    it('leaves samples below four eligible fares unchanged', () => {
      const observations = [5000, 5100, 25000].map((fare) => ({
        fare: { observedFare: fare },
      }));
      const result = filterFareOutliers(observations);

      expect(result.includedObservations).toHaveLength(3);
      expect(result.outlierObservations).toHaveLength(0);
    });
  });
  ...
  describe('3. Observation Eligibility & Exclusion Rules', () => {
    ...
    it('excludes connecting or unknown-stop observations when the production non-stop rule is enabled', () => {
      ...
      expect(isObservationEligibleForAggregation({ ...baseObservation, stops: 0 }, { requireNonStop: true })).toBe(true);
      expect(isObservationEligibleForAggregation({ ...baseObservation, stops: 1 }, { requireNonStop: true })).toBe(false);
      expect(isObservationEligibleForAggregation(baseObservation, { requireNonStop: true })).toBe(false);
    });
  });
